...
class GravityObjectLabelHandle : Editor
{
    ...
    private void Awake()
    {
        GravityObject gravityobject = (GravityObject)target;
        gravityobject.rBody = gravityobject.GetComponent<Rigidbody>();
    }
    ...
    void OnSceneGUI()
    {
        ...
        if (gravityobject.ForceShape == GravityObject.FieldShape.Sphere)
        {
            float arrowpos = gravityobject.Radius / 2f;
            float size = Mathf.Clamp(96, 0, arrowpos/2);
            float tr;
            ...
            if (gravityobject.GravityMultiplier < 0)
            {
                ...
                Handles.ArrowHandleCap(0, gravityobject.transform.position + (new Vector3(arrowpos, 0, 0)), Quaternion.LookRotation(Vector3.right), size, EventType.Repaint);
                Handles.ArrowHandleCap(0, gravityobject.transform.position + (new Vector3(0, arrowpos, 0)), Quaternion.LookRotation(Vector3.up), size, EventType.Repaint);
                Handles.ArrowHandleCap(0, gravityobject.transform.position + (new Vector3(0, 0, arrowpos)), Quaternion.LookRotation(Vector3.forward), size, EventType.Repaint);
                Handles.ArrowHandleCap(0, gravityobject.transform.position + (new Vector3(-(arrowpos), 0, 0)), Quaternion.LookRotation(-Vector3.right), size, EventType.Repaint);
                Handles.ArrowHandleCap(0, gravityobject.transform.position + (new Vector3(0, -(arrowpos), 0)), Quaternion.LookRotation(-Vector3.up), size, EventType.Repaint);
                Handles.ArrowHandleCap(0, gravityobject.transform.position + (new Vector3(0, 0, -(arrowpos))), Quaternion.LookRotation(-Vector3.forward), size, EventType.Repaint);
            }
        ...
        if (gravityobject.ForceShape == GravityObject.FieldShape.Cube)
        {
            float arrowposx = gravityobject.Size.x/2;
            float sizex = Mathf.Clamp(96, 0, arrowposx/2);
            float arrowposy = gravityobject.Size.y/2;
            float sizey = Mathf.Clamp(96, 0, arrowposy/2);
            float arrowposz = gravityobject.Size.z/2;
            float sizez = Mathf.Clamp(96, 0, arrowposz/2);
            ...
            //s.x = Handles.ScaleSlider(gravityobject.Size.x, Vector3.zero + (new Vector3(gravityobject.Size.x/2, 0, 0)), Vector3.right, Quaternion.identity, 3, 1f);
            if (EditorGUI.EndChangeCheck())
            {
            ...
            //s.y = Handles.ScaleSlider(gravityobject.Size.y, Vector3.zero + (new Vector3(0, gravityobject.Size.y / 2, 0)), Vector3.up, Quaternion.identity, 3, 1f);
            if (EditorGUI.EndChangeCheck())
            {
            ...
            //s.z = Handles.ScaleSlider(gravityobject.Size.z, Vector3.zero + (new Vector3(0, 0, gravityobject.Size.z / 2)), Vector3.forward, Quaternion.identity, 3, 1f);
            ...
            {
                Handles.color = new Color(0.9f, 0.9f, 0.9f, 0.75f);
                Handles.ArrowHandleCap(0, Vector3.zero + (new Vector3(arrowposx, 0, 0)), Quaternion.LookRotation(Vector3.right), sizex, EventType.Repaint);
                Handles.ArrowHandleCap(0, Vector3.zero + (new Vector3(0, arrowposy, 0)), Quaternion.LookRotation(Vector3.up), sizey, EventType.Repaint);
                Handles.ArrowHandleCap(0, Vector3.zero + (new Vector3(0, 0, arrowposz)), Quaternion.LookRotation(Vector3.forward), sizez, EventType.Repaint);
                Handles.ArrowHandleCap(0, Vector3.zero + (new Vector3(-(arrowposx), 0, 0)), Quaternion.LookRotation(-Vector3.right), sizex, EventType.Repaint);
                Handles.ArrowHandleCap(0, Vector3.zero + (new Vector3(0, -(arrowposy), 0)), Quaternion.LookRotation(-Vector3.up), sizey, EventType.Repaint);
                Handles.ArrowHandleCap(0, Vector3.zero + (new Vector3(0, 0, -(arrowposz))), Quaternion.LookRotation(-Vector3.forward), sizez, EventType.Repaint);
            }
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (gravityobject.ForceShape == GravityObject.FieldShape.Cube) { EditorGUILayout.PropertyField(Size,new GUIContent("Field Size:", "The size and shape of a Cubic force field.")); }
        if (gravityobject.ForceShape == GravityObject.FieldShape.Sphere) { EditorGUILayout.PropertyField(Radius,new GUIContent("Field Radius", "The maximum radius of the Spherical force field.")); }
