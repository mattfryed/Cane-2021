...
#define vd3 IN[2]
	...
	if (instID > (_instanceLod) - 1) {
		...
		o.uv.w = camDist > 0.003 ? fracFade : 1;
	...
	//SRP LIGHTING STUFF
	//
#if defined(SRP) && !defined(SRP_FRAGMENT_ADDITIONAL_LIGHTS)
...
FragmentOutput fragment_shader(g2f i) {
...
#elif defined(SHADOWS_SCREEN) || defined(SRP)
	...
	for (int lI = 0; lI < additionalLightsCount; ++lI) {
			...
			Light light = GetAdditionalLight(lI, i.worldPos, shadowMask);
