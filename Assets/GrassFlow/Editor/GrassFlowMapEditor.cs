...
namespace GrassFlow {
    public class GrassFlowMapEditor : EditorWindow {
        ...
        public static void Open(GrassMesh grassMesh, MapType mapType) {

            GrassFlowMapEditor mapCreator = EditorWindow.GetWindow<GrassFlowMapEditor>();

            mapCreator.grassMesh = grassMesh;
            ...
            switch(mapType) {
                case MapType.GrassColor:
                    ...
                    mapCreator.prevTex = grassMesh.colorMap;
                    grassMesh.colorMap = mapCreator.resultTex;
                    break;

                case MapType.GrassParameters:
                    mapCreator.minSize = new Vector2(512, 525);
                    mapCreator.prevTex = grassMesh.paramMap;
                    grassMesh.paramMap = mapCreator.resultTex;
                    break;

                case MapType.GrassType:
                    mapCreator.generateNoisyParameters = false;
                    mapCreator.minSize = new Vector2(512, 465);
                    mapCreator.prevTex = grassMesh.typeMap;
                    mapCreator.normalization1 = 0;
                    grassMesh.typeMap = mapCreator.resultTex;
                    break;
            }
        ...
        private void OnGUI() {
            ...
            if(GUILayout.Button("Create")) {
                ...
                switch(mapType) {
                    case MapType.GrassColor: grassMesh.colorMap = resultTex; break;
                    case MapType.GrassParameters: grassMesh.paramMap = resultTex; break;
                    case MapType.GrassType: grassMesh.typeMap = resultTex; break;
                }

                grassMesh.RefreshDetailMaps();
        ...
        private void OnDestroy() {
            if(!saveComplete) {
                switch(mapType) {
                    case MapType.GrassColor: grassMesh.colorMap = prevTex; break;
                    case MapType.GrassParameters: grassMesh.paramMap = prevTex; break;
                    case MapType.GrassType: grassMesh.typeMap = prevTex; break;
                }

                grassMesh.RefreshDetailMaps();

                grassMesh.owner.UpdateShader(grassMesh);
