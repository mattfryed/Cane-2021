...
namespace NGS.MeshFusionPro
{
    public class CombinedLODGroup : MonoBehaviour, ICombinedObject<CombinedLODGroupPart, LODGroupCombineSource>
    {
        ...
        public void Combine(IEnumerable<LODGroupCombineSource> sourceGroups)
        {
            ...
            if (_parts.Count == 0)
                InitBeforeFirstCombine(sources);
        ...
        {
            float div = _localBounds.size.magnitude / _originalBoundsSize;
            float prev = 1;

            div = Mathf.Clamp(div, 1, 100);
            ...
            for (int i = 0; i < _lodCount; i++)
            {
                ...
                lod.screenRelativeTransitionHeight = _originalSettings.screenTransitionsHeight[i] * div;

                if (lod.screenRelativeTransitionHeight >= prev)
                {
                    prev -= 0.03f;
                    lod.screenRelativeTransitionHeight = prev;
                }
